  ...
  const std::string& asset)
{
  ...
  for (int i = 0; i < layout.size(); ++i)
  {
    ...
    inputs[i].SemanticIndex = 
      (UINT) map_fetch(layout_item, "semanticIndex", jzero).as_number();

    inputs[i].Format = ParseDXGIFormat(layout_item["format"].as_string());

    inputs[i].InputSlot = 
      (UINT) map_fetch(layout_item, "inputSlot", jzero).as_number();
    ...
    {
      inputs[i].AlignedByteOffset = (UINT) alignment.as_number();
    }
    ...
    inputs[i].InstanceDataStepRate =
      (UINT) map_fetch(layout_item, "dataStepRate", jzero).as_number();
  }

  auto hr = device->Device->
    CreateInputLayout(inputs, (UINT) layout.size(),
