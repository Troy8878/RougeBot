...
#pragma region Basics
  ...
  void value::shared_value::set_type(json_type new_type, const void *copy)
  {
    ...
    switch (type)
    {
      ...
      default:
        throw basic_exception("Invalid json type encountered (corrupt memory?)");
    }
  }
  ...
  static bool read_char(std::istream &input, std::string &out)
  {
    ...
    if (val == '\\')
    {
      ...
      switch (val)
      {
        ...
        case 'u':
          input.read(buf, 4);
          wbuf[0] = static_cast<wchar_t>(std::stoul(buf, nullptr, 16));
          out = narrow(wbuf);
          break;
        case 'U':
          input.read(buf, 8);
          char32_t cp[2] = {std::stoul(buf, nullptr, 16), 0};
...
#pragma region Serialize
  ...
  struct serialize_method_manager
  {
    ...
    serialize_fn serialize_fns[static_cast<uint64_t>(json_type::num_types)];

    serialize_fn operator[](json_type type)
    {
      assert(static_cast<uint64_t>(type) < static_cast<uint64_t>(json_type::num_types));
      return serialize_fns[static_cast<uint64_t>(type)];
    }

    serialize_method_manager()
    {
      serialize_fns[static_cast<uint64_t>(json_type::jnull)] = &value::serialize_null;
      serialize_fns[static_cast<uint64_t>(json_type::jobject)] = &value::serialize_object;
      serialize_fns[static_cast<uint64_t>(json_type::jarray)] = &value::serialize_array;
      serialize_fns[static_cast<uint64_t>(json_type::jstring)] = &value::serialize_string;
      serialize_fns[static_cast<uint64_t>(json_type::jnumber)] = &value::serialize_number;
      serialize_fns[static_cast<uint64_t>(json_type::jbool)] = &value::serialize_bool;
    }
  };
